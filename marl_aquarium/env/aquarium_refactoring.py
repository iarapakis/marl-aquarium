...
class raw_env(ParallelEnv[str, Box, Discrete | None]):  # pylint: disable=C0103
    ...
    def get_obs(self):
        ...
        observations = prey_observations | predator_observations # merge both dicts
        return observations
    ...
    def get_n_closest_entities(self, observer: Entity, entities: Sequence[Entity], n: int) -> List[Entity]:
        ...
        distances = [
            (self.torus.get_distance_in_torus(observer.position, entity.position), entity)
        ...
        n_nearest = sorted_distances[:n]
        nearest_entities = [entity for (_, entity) in n_nearest]
        return nearest_entities

    def predator_get_n_closest_fish(self, observer: Entity) -> List[Entity]:
        """Get the n nearest fish to the observer."""
        return self.get_n_closest_entities(observer, self.prey, self.prey_observe_count)

    def prey_get_n_closest_animals(self, observer: Entity, other_animals: Sequence[Entity], n_nearest_animals: int) -> List[Entity]:
        """Get the n nearest animals to the observer."""
        return self.get_n_closest_entities(observer, other_animals, n_nearest_animals)

    def prey_nearby_animal_observations(self, observer: Entity, all_animals: Sequence[Entity], n_nearest_animal: int, animal_type: str) -> List[float]:
        """Get the observations of the current state of the environment of an observer."""
        observations = []
        if self.fov_enabled:
            for animal in all_animals:
                if self.torus.check_if_entity_is_in_view_in_torus(
                    observer, animal, self.prey_view_distance, self.prey_fov
                ):
                    observation = self.nearby_animal_observation(observer, animal)
                    if observation is not None:
                        observations += observation
        else:
            closest_animals = self.prey_get_n_closest_animals(
                observer, all_animals, n_nearest_animal
            )
            for animal in closest_animals:
                if animal_type == 'fish' and animal is observer:
                    continue
                observation = self.nearby_animal_observation(observer, animal)
                if observation is not None:
                    observations += observation

        while len(observations) < n_nearest_animal * self.obs_size:
            observations += [0] * self.obs_size

        assert len(observations) == n_nearest_animal * self.obs_size
        return observations
    ...
    def get_prey_observations(self, observer: Entity):
        ...
        observed_sharks = self.prey_nearby_animal_observations(
            observer, self.predators, self.predator_observe_count, 'shark'
        )

        observed_fishes = self.prey_nearby_animal_observations(
            observer, self.prey, self.prey_observe_count, 'fish'
        )
    ...
    def predator_nearby_animal_observations(self, observer: Entity, animals: Sequence[Entity], observe_count: int, fov_enabled: bool, get_closest_animals_func=None) -> Sequence[float]:
        """Get the observations of the current state of the environment of an observer."""
        observations = []
        if fov_enabled:
            for animal in animals:
                if (
                    ...
                    and len(observations) < observe_count * self.obs_size
                ):
                    observation = self.nearby_animal_observation(observer, animal)
                    observations += observation
        else:
            closest_animals = get_closest_animals_func(observer)
            for animal in closest_animals:
                if animal is not observer:
                    observation = self.nearby_animal_observation(observer, animal)
                    observations += observation

        if len(observations) < observe_count * self.obs_size:
            observations += [0] * (observe_count * self.obs_size - len(observations))

        return observations

    def predator_nearby_shark_observations(self, observer: Entity) -> Sequence[float]:
        return self.predator_nearby_animal_observations(observer, self.predators, self.predator_observe_count, self.fov_enabled)

    def predator_nearby_fish_observations(self, observer: Entity) -> Sequence[float]:
        return self.predator_nearby_animal_observations(observer, self.prey, self.prey_observe_count, self.fov_enabled, self.predator_get_n_closest_fish)
